...
<select name="pw_content">
	...
	<?php for ( $i = 1; $i <= 5; $i++ ) : ?>
	<option value="<?php echo $i; ?>" <?php selected( $i, $post->pw_content ); ?>><?php echo __( 'Content', 'tcd-w' ) . $i . ' : ' . esc_html( $dp_options['pw_name' . $i] ); ?></option>
	<?php endfor; ?>
</select>
<?php
}
...
function tcd_password_form() {
	...
	if ( $pw_content ) {
		...
		// ボタンを追加
		if ( $dp_options['pw_btn_display' . $pw_content] ) {

			$target = $dp_options['pw_btn_target' . $pw_content] ? ' target="_blank"' : '';
			...
			$output .= '<a class="c-pw__btn c-pw__btn--register" href="' . esc_url( $dp_options['pw_btn_url' . $pw_content] ) . '"' . $target . '>' . esc_html( $dp_options['pw_btn_label' . $pw_content] ) .'</a></div>';
	...
	if ( ! $pw_content ) {
		...
		if ( $dp_options['pw_label'] ) {
			$output .= '<label class="c-pw__box-label" for="' . $label . '">' . esc_html( $dp_options['pw_label'] ) . '</label>';
		}
