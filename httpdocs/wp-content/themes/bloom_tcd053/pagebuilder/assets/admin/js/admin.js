jQuery(document).ready(function($){
	...
	var window_scroll_disable = function() {
		...
		// エディター選択等のmoreポップアップメニューが開いていれば閉じる
		// WordPress 6.6
		if ($('.more-menu-dropdown__content').length) {
			$('.editor-header__settings .components-dropdown-menu:last .components-dropdown-menu__toggle').trigger('click');
		// WordPress 6.4
		} else if ($('.interface-more-menu-dropdown__content').length) {
			$('.interface-more-menu-dropdown button:first').trigger('click');
		}
	};
	...
	if ($('.block-editor__container').length) {
		...
		// ヘッダー右の詳細ボタン等のセレクタ
		var be_selectors = {
			more_menu_button : '.edit-post-more-menu button:first',
			more_menu_content : '.edit-post-more-menu__content',
			editor_button : '.edit-post-more-menu__content button'
		};

		// Block editor初期化待ってから実行
		wp.domReady(function(){
			setTimeout(function(){
				// ページビルダー有効時かつ集中執筆モードがオンの場合は集中執筆モードをオフにする 読み込み時及び常時監視
				var check_distraction_free = function() {
					if ($pb_metabox.find('[name="use_page_builder"]').val() == 1 && wp.data.select('core/preferences').get('core', 'distractionFree')) {
						wp.data.dispatch('core/editor').toggleDistractionFree();
					}
				};
				check_distraction_free();
				setInterval(check_distraction_free, 500);

				// WordPress 6.6
				if ($editor.find('.editor-header__settings').length) {
					be_selectors = {
						more_menu_button : '.editor-header__settings .components-dropdown-menu:last .components-dropdown-menu__toggle',
						more_menu_content : '.more-menu-dropdown__content',
						editor_button : '.more-menu-dropdown__content button'
					};
				// WordPress 6.0
				} else if (!$editor.find('.edit-post-more-menu').length) {
					be_selectors = {
						more_menu_button : '.interface-more-menu-dropdown button:first',
						more_menu_content : '.interface-more-menu-dropdown__content',
						editor_button : '.interface-more-menu-dropdown__content button'
					};
				}

				// ヘッダー右の詳細ボタンクリック
				$editor.on('click', be_selectors.more_menu_button, function(){
					setTimeout(function(){
						// ポップメニューが開いてなければ終了
						if (!$(be_selectors.more_menu_content).length) return;
						...
						// ページビルダー有効・無効に応じて集中執筆ボタンを表示切替する関数
						var toggle_distraction_free_button = function() {
							var $distraction_free_button = $(be_selectors.editor_button).filter(function() {
								var buttonText = $(this).text();
								return buttonText.indexOf('集中執筆') > -1 || buttonText.indexOf('Distraction free') > -1 || buttonText.indexOf('Distraction Free') > -1;
							});
							if ($pb_metabox.find('[name="use_page_builder"]').val() == 1) {
								$distraction_free_button.hide();
							} else {
								$distraction_free_button.show();
							}
						};

						// コードエディターボタン取得
						if (pagebuilder_i18n.code_editor && $(be_selectors.editor_button + ':contains("'+pagebuilder_i18n.code_editor+'")').length) {
							$code_editor_select_button = $(be_selectors.editor_button + ':contains("'+pagebuilder_i18n.code_editor+'")');
						} else if ($(be_selectors.editor_button + ':contains("コードエディター")').length) {
							$code_editor_select_button = $(be_selectors.editor_button + ':contains("コードエディター")');
						} else if ($(be_selectors.editor_button + ':contains("Code editor")').length) {
							$code_editor_select_button = $(be_selectors.editor_button + ':contains("Code editor")');
						} else if ($(be_selectors.editor_button + ':contains("Code Editor")').length) {
							$code_editor_select_button = $(be_selectors.editor_button + ':contains("Code Editor")');
						}
						...
						// チェックありの既存エディター取得
						$current_editor_button = $select_button_parent.find('.has-icon, .components-icon-button, [aria-checked=true]');
						...
						// ページビルダー有効時
						if ($pb_metabox.find('[name="use_page_builder"]').val() == 1) {
							// チェックありの既存エディター以外を非表示
							$select_button_parent.children().not($current_editor_button).hide();

							// 24pxチェックあり
							var $svg = $current_editor_button.find('svg[width="24"][height="24"]')
							if ($svg.length) {
								// WordPress 6.4
								if ($svg.hasClass('has-icon-right')) {
									$pb_select_button = $('<button type="button" aria-checked="true" role="menuitemradio" class="components-button components-menu-item__button components-menu-items-choice"><span class="components-menu-item__item">' + pb_select_button_label + '</span><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" class="components-menu-items__item-icon has-icon-right" aria-hidden="true" focusable="false"><path d="M16.7 7.1l-6.3 8.5-3.3-2.5-.9 1.2 4.5 3.4L17.9 8z"></path></svg></button>');
								// WordPress 5.6
								} else if (!$current_editor_button.hasClass('has-icon')) {
									$pb_select_button = $('<button type="button" aria-checked="true" role="menuitemradio" class="components-button components-menu-item__button components-menu-items-choice">' + pb_select_button_label + '<svg width="24" height="24" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="components-menu-items__item-icon" role="img" aria-hidden="true" focusable="false"><path d="M18.3 5.6L9.9 16.9l-4.6-3.4-.9 1.2 5.8 4.3 9.3-12.6z"></path></svg></button>');
								// WordPress 5.5
								} else {
						...
						// エディター変更ボタンにページビルダーボタン追加
						$select_button_parent.append($pb_select_button);

						// エディター選択ボタンクリック
						$(be_selectors.editor_button).off('click.pb_editor_button').on('click.pb_editor_button', function(){
							switch ($(this).text()) {
								...
								case 'ページビルダー' :
									if ($pb_metabox.find('[name="use_page_builder"]').val() != 1) {
										...
										// 手動でポップアップ閉じる
										$editor.find(be_selectors.more_menu_button).trigger('click');
									}

									// 集中執筆ボタン表示切替
									toggle_distraction_free_button();
