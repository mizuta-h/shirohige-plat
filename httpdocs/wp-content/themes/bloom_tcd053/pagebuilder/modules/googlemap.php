...
function get_page_builder_widget_googlemap_default_values() {
	...
	$default_values = array(
		...
		'overlay_headline_font_family' => 1,
...
<div class="form-field form-field-overlay hidden">
	...
	<table style="margin-top:5px;">
		...
		<tr>
			...
			<td>
			<?php echo UI\font_select( 'pagebuilder[widget][' . esc_attr($values['widget_index']) . '][overlay_headline_font_family]', $values['overlay_headline_font_family'] ); ?>
			</td>
		</tr>
		<tr>
...
<div class="form-field form-field-overlay hidden">
	...
	<table style="margin-top:5px;">
		...
		<tr>
			<td><?php _e('Font family', 'tcd-w'); ?></td>
			<td>
			<?php echo UI\font_select( 'pagebuilder[widget][' . esc_attr($values['widget_index']) . '][overlay_content_font_family]', $values['overlay_content_font_family'] ); ?>
			</td>
		</tr>
		<tr>
			<td><?php _e('Text align', 'tcd-w'); ?></td>
...
function display_page_builder_widget_googlemap($values = array(), $widget_index = null) {
	...
	if ( ! empty( $values['show_overlay'] ) ) { // Use overlay
		...
		$overlay_headline_font_family_raw = $values['overlay_headline_font_family'];
		$overlay_content_font_family_raw = $values['overlay_content_font_family'];
		$map = [
		 'type1' => 1,
		 'type2' => 1,
		 'type3' => 2,
	   ...
	   // 不明な値は 1 にフォールバック
	   $overlay_headline_font_family = $map[$overlay_headline_font_family_raw] ?? 1;
	   $overlay_content_font_family = $map[$overlay_content_font_family_raw] ?? 1;

		if ($values['overlay_headline']) {
			$overlay_contents['headline'] = '<h3 class="pb_googlemap_headline pb_font_family_'.esc_attr($overlay_headline_font_family).'">'.str_replace(array("\r\n", "\r", "\n"), '<br>', esc_html($values['overlay_headline'])).'</h3>';
		}

		if ($values['overlay_content']) {
			$overlay_contents['content'] = '<div class="pb_googlemap_content pb_font_family_'.esc_attr($overlay_content_font_family).'">'.str_replace(array("\r\n", "\r", "\n"), '<br>', esc_html($values['overlay_content'])).'</div>';
		}
...
<script>jQuery(window).on('load', function() { initMap('js-googlemap-<?php echo esc_js( $widget_index ); ?>', '<?php echo esc_js( $values['map_address'] ); ?>', <?php echo esc_js( $saturation ); ?>, <?php echo esc_js( $use_custom_overlay ); ?>, '<?php echo esc_js( $marker_img ); ?>', '<?php echo esc_js( $marker_text ); ?>');});</script><?php
}
