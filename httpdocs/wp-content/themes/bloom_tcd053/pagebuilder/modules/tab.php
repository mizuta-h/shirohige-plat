			...
			<div class="form-field">
				...
				<table style="margin-top:5px;">
					...
					<tr>
						...
						<td>
						<?php echo UI\font_select( esc_attr($repeater_level2_name_base) . '[headline_font_family]', $repeater_level2_values['headline_font_family'] ); ?>
						</td>
					</tr>
					<tr>
			...
			<div class="form-field">
				...
				<table style="margin-top:5px;">
					...
					<tr>
						<td><?php _e('Font family', 'tcd-w'); ?></td>
						<td>
						<?php echo UI\font_select( esc_attr($repeater_level2_name_base) . '[content_font_family]', $repeater_level2_values['content_font_family'] ); ?>
						</td>
					</tr>
					<tr>
						<td><?php _e('Text align', 'tcd-w'); ?></td>
...
<?php
			...
			if (!empty($repeater_values['row2_index']) && is_array($repeater_values['row2_index'])) {
				...
				foreach($repeater_values['row2_index'] as $key => $repeater_level2_index) {
					...
					$headline_font_family_raw = $repeater_level2_values['headline_font_family'];
					$content_font_family_raw = $repeater_level2_values['content_font_family'];
					$map = [
					 'type1' => 1,
					 'type2' => 1,
					 'type3' => 2,
				   ...
				   // 不明な値は 1 にフォールバック
				   $headline_font_family = $map[$headline_font_family_raw] ?? 1;
				   $content_font_family = $map[$content_font_family_raw] ?? 1;

					if (!empty($repeater_level2_values['headline'])) {
						$content_html .= '<h3 class="pb_tab_headline pb_font_family_'.esc_attr($headline_font_family).'">'.str_replace(array("\r\n", "\r", "\n"), '<br>', esc_html($repeater_level2_values['headline'])).'</h3>';
					}

					if (!empty($repeater_level2_values['content'])) {
						$content_html .= '<div class="pb_tab_description pb_font_family_'.esc_attr($content_font_family).'">'.str_replace(array("\r\n", "\r", "\n"), '<br>', esc_html($repeater_level2_values['content'])).'</div>';
					}
	...
	if ($post_widgets) {
		...
		foreach($post_widgets as $post_widget) {
			...
			foreach($repeater_indexes as $repeater_index) {
				...
				if ( isset($values['repeater'][$repeater_index]) && is_array($values['repeater'])) {
					$repeater_values = $values['repeater'][$repeater_index];
				}
				$repeater_values = array_merge($default_row_values, $repeater_values);
...
function page_builder_widget_tab_has_slick($content_type = null) {
	...
	if ($post_widgets) {
		foreach($post_widgets as $post_widget) {
			...
			foreach($repeater_indexes as $repeater_index) {
				$repeater_values = array();
				if (isset($values['repeater'][$repeater_index]) && is_array($values['repeater'][$repeater_index])) { // キーが存在し、配列か確認
					$repeater_values = $values['repeater'][$repeater_index];
