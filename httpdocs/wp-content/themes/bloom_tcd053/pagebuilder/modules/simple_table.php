	...
	<div class="pb_repeater_content">
		...
		<div class="pb_repeater_field">
			...
			<div class="content_type-type2"<?php if ($row_values['content_type'] != 'type2') echo ' style="display:none;"'; ?>>
				...
				<div class="form-field">
					<h4><?php _e('TikTok URL', 'tcd-w'); ?></h4>
					<input type="text" name="pagebuilder[widget][<?php echo esc_attr($values['widget_index']); ?>][repeater][<?php echo esc_attr($values['repeater_index']); ?>][tiktok_url]" value="<?php echo esc_attr($row_values['tiktok_url']); ?>" />
				</div>
				<div class="form-field">
					<h4><?php _e('X URL', 'tcd-w'); ?></h4>
...
function display_page_builder_widget_simple_table($values = array(), $widget_index = null, $widget = array(), $post_id = null) {
	...
	foreach($repeater_indexes as $repeater_index) {
		...
		$repeater_values += array('tiktok_url' => '');
		echo '<tr>';
		...
		if ($repeater_values['content_type'] == 'type2') {
			...
			if ($repeater_values['tiktok_url']) {
				$sns_html .= '<li class="pb_simple_table_icon-tiktok"><a href="' . esc_attr( $repeater_values['tiktok_url'] ) . '" target="_blank"><span>TikTok</span></a></li>';
			}
			if ($repeater_values['twitter_url']) {
				$sns_html .= '<li class="pb_simple_table_icon-twitter"><a href="' . esc_attr( $repeater_values['twitter_url'] ) . '" target="_blank"><span>X</span></a></li>';
			}
